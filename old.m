...
dv_tg_Train = kron(dv_tg, ones(1,len));

tr_tg = target_data(:,tr_index);
tr_tg_Train = kron(tr_tg, ones(1,len));
                ...
                for m = 1:length(a)
                    for n = 1:length(lambda_r)
                        ...
                        for seed = 1:r_guesses
                            ...
                            hidden_tr_Train = zeros(Nh(k),0);

                            for sample=1:size(tr_in,3)
                                [~, sequence_tr, pooler_tr] = rc(tr_in(:,:,sample), seed, omega_in(i), rho(j), Nh(k), dns(l), a(m), ws);
                                hidden_tr_Train = cat(2,hidden_tr_Train,sequence_tr);
                                hidden_tr = cat(2,hidden_tr,pooler_tr);
                            end
                            W_out = trainOffline(hidden_tr_Train,tr_tg_Train, lambda_r(n), ws);
...
disp('Refit')
hidden_dv_Train = zeros(Nh_best,0);
for sample=1:size(dv_in,3)
    [~, sequence_dv] = rc(dv_in(:,:,sample), seed, omega_in_best, rho_best, Nh_best, dns_best, a_best, ws);
    hidden_dv_Train = cat(2,hidden_dv_Train, sequence_dv);
end
W_out_best = trainOffline(hidden_dv_Train, dv_tg_Train, lambda_r_best, ws);
...
save(fullfile('results', strcat('LIESN_hyperparameters', '.mat')), 'seed', 'omega_in_best', 'rho_best', 'Nh_best', 'dns_best', 'a_best', 'lambda_r_best')
